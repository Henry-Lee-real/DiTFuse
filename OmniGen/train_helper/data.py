...
class DatasetFromJson_Combine(torch.utils.data.Dataset):
    def __init__(
        self,
        json_file_A: str,  # Input file A
        json_file_B: str,  # Input file B
        image_path: str,
        processer: OmniGenProcessor,
        image_transform,
        max_input_length_limit: int = 18000,
        ...
        self.data_A = load_dataset('json', data_files=json_file_A)['train']
        self.data_B = load_dataset('json', data_files=json_file_B)['train']
        
        # Shuffle data within each file
        self.data_A = self.data_A.shuffle(seed=42)
        self.data_B = self.data_B.shuffle(seed=42)
        
        # Combine A and B, keeping A first
        self.data=concatDataset([self.data_A, self.data_B])
        self.image_path = image_path
